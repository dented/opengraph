...
module OpenGraph
  ...
  def self.fetch(uri, strict = true, opts = {})
    options = { timeout: 30 }.merge(opts)
    resource = RestClient::Resource.new(uri, open_timeout: options[:timeout])
    parse(resource.get.body, strict)
